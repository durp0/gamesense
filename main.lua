...
for i,v in next, images do
    if not isfile(library.folder..'/assets/'..i..'.ln') then
        writefile(library.folder..'/assets/'..i..'.ln', game:HttpGet(v))
    end
    images[i] = readfile(library.folder..'/assets/'..i..'.ln')
end
...
local test = rage:section({name = "Test Section"})
test:toggle({}):colorpicker({})
test:toggle({}):keybind({})
test:slider({})
test:dropdown({})
test:button({})
